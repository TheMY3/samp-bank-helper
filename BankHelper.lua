...
if not doesFileExist('moonloader/config/auto_pd.ini') then inicfg.save(mainIni, 'auto_pd.ini') end
...
function imgui.OnDrawFrame()
	...
	if menu == 1 then
		...
		if imgui.Button(u8'Сохранить все настройки',imgui.ImVec2(580,30))then saveCFG() end
	end
	if menu == 2 then
...
function onScriptTerminate(script, quitGame)
	...
	if script == thisScript() then -- зададим условие что именно текущий скрипт завершает работ
